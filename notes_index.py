...
What this platform does that surprises you was written down in several places
that nobody reads together. This indexes three of them:

  Supply Chain Mgt/docs/platform-notes.md   platform notes, as SC- ids
  Defense/README.md, section 13             a defect log, as DF- ids
  learnings/VANTIQ-LEARNINGS-*.md           the pooled corpus: DM-, NC-, NR-, PS-

The Logistics demo's README keeps an earlier defect log in prose, which the
platform notes describe themselves as additions to. It is not indexed.
...
COVERAGE and POOLED_COVERAGE are the only hand-maintained parts, and they are
deliberately honest: most notes are NOT enforceable, and saying so is more
useful than implying the harness has them covered.
"""
import io
import os
import re
...
def cell(title, limit, table):
    """One title as a table cell or list item.

    Two defects this replaces. Titles were cut at a fixed width with no
    ellipsis - 82 of them, 55 mid-word - and `|` was rewritten to `/` so it could not
    break a table, which turned DM-07's `||` into `//` and changed what the
    entry says. A cut now lands on a word, never inside inline code, and says
    so; a pipe is escaped only in a table, since a list item needs no escaping
    and GitHub would print the backslash.
    """
    ...
    if len(title) > limit:
        ...
        if cut.count("`") % 2:
            cut = cut[:cut.rfind("`")]
        title = cut.rstrip(" ,;:-") + "\u2026"
    return title.replace("|", "\\|") if table else title
...
def build():
    ...
    out = []
    # Blank lines around every block. GitHub renders a table that sits directly
    # under a paragraph, but that is GitHub being lenient; other Markdown
    # renderers need the blank line.
    ...
    out.append("Generated by `notes_index.py` from the places seven demos wrote "
               ...
               "and re-run.\n\n")
    out.append("| | demo series | pooled corpus | total |\n|---|---|---|---|\n")
    ...
    out.append("\n## Enforced, from the demo series\n\n")
    out.append("| id | Learning | Layer | Check |\n|---|---|---|---|\n")
    for _l, ident, title, where, what in enforced:
        out.append("| %s | %s | `%s` | `%s` |\n"
                   % (ident, cell(title, 100, True), where, what))

    out.append("\n## Convention only, from the demo series\n\n")
    ...
    for _l, ident, title, _w, what in convention:
        out.append("| %s | %s | %s |\n" % (ident, cell(title, 100, True), what))
    ...
    if p_convention:
        ...
        out.append("These cannot be detected statically either, and belong in the "
                   "same places.\n\n")
        out.append("| id | Learning | What to do |\n|---|---|---|\n")
